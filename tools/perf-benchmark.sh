...
cleanup() {
  ...
  if [ -n "$generated_workload_dir" ] && [ -d "$generated_workload_dir" ]; then
    rm -rf "$generated_workload_dir"
  fi
}
...
find_riscv_tool() {
  ...
  if [ -n "$value" ]; then
    if [ -x "$value" ] || command -v "$value" >/dev/null 2>&1; then
      command -v "$value" 2>/dev/null || printf '%s\n' "$value"
      return
    fi
    echo "$env_name is set but not executable: $value" >&2
    exit 1
  fi
  ...
  echo "RISC-V tool not found: ${*}" >&2
  exit 1
}

build_fp32_hotloop_workload() {
  local cc objcopy asm elf bin

  cc=$(find_riscv_tool RISCV_CC riscv64-unknown-elf-gcc riscv64-linux-gnu-gcc)
  objcopy=$(find_riscv_tool RISCV_OBJCOPY riscv64-unknown-elf-objcopy riscv64-linux-gnu-objcopy)

  if [ -z "$generated_workload_dir" ]; then
    generated_workload_dir=$(mktemp -d)
  fi

  asm="$generated_workload_dir/fp32-hotloop.S"
  elf="$generated_workload_dir/fp32-hotloop.elf"
  bin="$generated_workload_dir/fp32-hotloop.bin"
...
EOF
  ...
  "$cc" -x assembler-with-cpp -nostdlib -nostartfiles -march=rv64gc -mabi=lp64d \
    -Wl,-Ttext=0x80000000 -o "$elf" "$asm"
...
fp32_hotloop=$(build_fp32_hotloop_workload)
...
append_result_row "fp32-hotloop" "$fp32_hotloop"
append_suite_result_row "fp32-riscv-tests-suite" "${fp32_tests[@]}"
...
* fp32-hotloop is generated by this script to stress scalar FP32 arithmetic.
* fp32-riscv-tests-suite aggregates selected scalar AM rv64uf riscv-tests binaries.
