...
#define Logf(flag, ...) \
  do { \
    if (flag == dflag_mem             && log_bool  ) Log(__VA_ARGS__); \
    if (flag == dflag_translate       && log_bool  ) Log(__VA_ARGS__); \
    if (flag == dflag_trace_inst      && log_bool  ) Log(__VA_ARGS__); \
    if (flag == dflag_trace_inst_dasm && log_bool  ) Log(__VA_ARGS__); \
    if (flag == dflag_trace_bb        && log_bool  ) Log(__VA_ARGS__); \
    if (flag == dflag_exit            && log_bool  ) Log(__VA_ARGS__); \
    if (flag == dflag_simpoint        && log_bool  ) Log(__VA_ARGS__); \
  } while (0)
