...
#define Logf(flag, ...) \
  do { \
    if (flag == dflag_mem) Log(__VA_ARGS__); \
    if (flag == dflag_translate) Log(__VA_ARGS__); \
    if (flag == dflag_trace_inst) Log(__VA_ARGS__); \
    if (flag == dflag_trace_inst_dasm) Log(__VA_ARGS__); \
