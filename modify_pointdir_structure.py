    ...
    for cidx in cpt_idxs:
        ...
        for cpt_file in cpt_files:
            if cpt_file.endswith('gz'):
                ...
                sdbin_path = os.path.join(cpt_dir, f'sdcard.bin')
                new_sdbin_path = os.path.join(new_cpt_dir, f'sdcard.bin')
                # print(f'mv {sdbin_path} {new_sdbin_path}')
                shutil.move(sdbin_path, new_sdbin_path)
