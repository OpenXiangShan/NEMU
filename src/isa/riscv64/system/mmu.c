...
#else
    pte.val	= paddr_read(p_pte, PTE_SIZE, MEM_TYPE_READ, MODE_S);
#endif
...
#ifdef XIANGSHAN_DEBUG
  printf("[NEMU] pmp out_mode:%d cpu.mode:%ld ifetch:%d mprv:%d mpp:%d mode:%d", out_mode, cpu.mode, ifetch, mstatus->mprv, mstatus->mpp, mode);
#endif
