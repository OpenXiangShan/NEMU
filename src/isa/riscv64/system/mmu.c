...
paddr_t gpa_stage(paddr_t gpaddr, vaddr_t vaddr, int type){
  ...
  if(hgatp->mode == 8){
...
#if ENABLE_HOSTTLB
    extern paddr_t hosttlb_guest_lookup(paddr_t vaddr, int type);
    res_addr = hosttlb_guest_lookup(gpaddr, type);
    if (res_addr != MEM_RET_FAIL) {
      return res_addr;
    }
#endif
    ...
    `   extern void hosttlb_guest_insert(paddr_t gpaddr, paddr_t paddr, int type);
        hosttlb_guest_insert(gpaddr, res_addr, type);
#endif
        return res_addr;
      }
    }
