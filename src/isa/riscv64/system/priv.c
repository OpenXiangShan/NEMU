...
void dasics_fetch_helper(vaddr_t pc, vaddr_t prev_pc, uint8_t cfi_type) {
  ...
  bool src_activezone = dasics_match_djumpbound(prev_pc, JUMPCFG_V);
  bool dst_activezone = dasics_match_djumpbound(pc, JUMPCFG_V);

  Logm("[Dasics Fetch] prev_pc: 0x%lx (T:%d F:%d), pc:0x%lx (T:%d F:%d)\n", prev_pc, src_trusted, src_activezone, pc, dst_trusted, dst_activezone);
  ...
  bool allow_lib_to_lib = src_activezone && dst_activezone;

  bool allow_br   = src_trusted || dst_activezone;
  bool allow_jump = src_trusted || allow_lib_to_main || allow_lib_to_lib;

  bool allow_cfi = (cfi_type == CFI_BRANCH && allow_br) || (cfi_type == CFI_JUMP && allow_jump);

  if (!allow_cfi) {
    INTR_TVAL_REG(EX_DUIAF) = pc;
    Logm("Dasics fetch exception occur: pc%lx  (st:%d,dl:%d)\n",pc,src_trusted,dst_activezone);
