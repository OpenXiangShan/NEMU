...
inline void update_vstopi() {
  ...
  uint16_t iidOnlyC2 = IRQ_SEIP;
  uint16_t iidOnlyC4 = vs_iid_num;
  uint16_t iidOnlyC5 = hvictl->iid;

  uint16_t hvictlDPR = hvictl->dpr ? 0xff : 0;

  vstopei_t vstopei_tmp = (vstopei_t)cpu.fromaia.vstopei;
  bool iprioC1GreaterThan255 = vstopei_tmp.iprio > 0xff;
  bool iprioC4IsZero = vs_prio_num == 0;
  bool iprioC2C5IsZero = hvictl->iprio == 0;
  ...
  uint8_t iprioC4Tmp = iprioC4IsZero ?
                        vs_iid_idx > get_prio_idx_in_group(IRQ_VSEIP) ? 0xff : 0
                        : iprioC4;
  uint8_t iprioC3C5Tmp = iprioC2C5IsZero ? hvictlDPR : iprioC2C5;

  uint8_t iprioOnlyC1 = iprioC1GreaterThan255 ? 0xff : iprioC1Tmp;
  ...
  if (iprioC1 < iprioC4) {
    ...
    if (get_prio_idx_in_group(IRQ_SEIP) < vs_iid_idx) {
      iidC1C4 = iidOnlyC1;
      iprioC1C4 = iprioC1Tmp;
    } else {
      iidC1C4 = iidOnlyC4;
    ...
    iidC1C4 = iidOnlyC4;
    if (iprioC4IsZero) {
      if (vs_iid_idx > get_prio_idx_in_group(IRQ_VSEIP)) {
        iprioC1C4 = 0xff;
      }
    } else {
      iprioC1C4 = iprioC4;
  ...
  if (candidate_no_valid) {
    ...
    } else if (!candidate1 & !candidate2 & candidate4) {
      vstopi->iid = iidOnlyC4;
    } else if (!candidate1 & !candidate2 & candidate5) {
