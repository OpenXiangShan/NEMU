...
#define def_fop_template(name, op, w) \
  def_EHelper(name) { \
    rtl_fpcall(s, ddest, dsrc1, dsrc2, FPCALL_CMD(op, w)); \
...
def_EHelper(fsqrts) {
  rtl_fpcall(s, ddest, dsrc1, rz, FPCALL_CMD(FPCALL_SQRT, FPCALL_W32));
  rtl_fsr(s, ddest, ddest, FPCALL_W32);
}

def_EHelper(fmadds) {
  rtl_mv(s, s0, &fpreg_l(s->isa.instr.fp.funct5)); // rs3
  rtl_fpcall(s, s0, dsrc1, dsrc2, FPCALL_CMD(FPCALL_MADD, FPCALL_W32));
...
def_EHelper(fnmsubs) {
  ...
  rtl_fpcall(s, s0, s1, dsrc2, FPCALL_CMD(FPCALL_MADD, FPCALL_W32));
  rtl_fsr(s, ddest, s0, FPCALL_W32);
}
...
def_EHelper(fles) {
  rtl_fpcall(s, ddest, dsrc1, dsrc2, FPCALL_CMD(FPCALL_LE, FPCALL_W32));
}

def_EHelper(flts) {
  rtl_mv(s, ddest, dsrc1);
  rtl_fpcall(s, ddest, dsrc1, dsrc2, FPCALL_CMD(FPCALL_LT, FPCALL_W32));
}

def_EHelper(feqs) {
  rtl_mv(s, ddest, dsrc1);
  rtl_fpcall(s, ddest, dsrc1, dsrc2, FPCALL_CMD(FPCALL_EQ, FPCALL_W32));
}

def_EHelper(fcvt_s_w) {
  rtl_fpcall(s, ddest, dsrc1, rz, FPCALL_CMD(FPCALL_I32ToF, FPCALL_W32));
  rtl_fsr(s, ddest, ddest, FPCALL_W32);
}

def_EHelper(fcvt_s_wu) {
  rtl_fpcall(s, ddest, dsrc1, rz, FPCALL_CMD(FPCALL_U32ToF, FPCALL_W32));
  rtl_fsr(s, ddest, ddest, FPCALL_W32);
}

def_EHelper(fcvt_s_l) {
  rtl_fpcall(s, ddest, dsrc1, rz, FPCALL_CMD(FPCALL_I64ToF, FPCALL_W32));
  rtl_fsr(s, ddest, ddest, FPCALL_W32);
}

def_EHelper(fcvt_s_lu) {
  rtl_fpcall(s, ddest, dsrc1, rz, FPCALL_CMD(FPCALL_U64ToF, FPCALL_W32));
  rtl_fsr(s, ddest, ddest, FPCALL_W32);
}

def_EHelper(fcvt_w_s) {
  rtl_fpcall(s, ddest, dsrc1, rz, FPCALL_CMD(FPCALL_FToI32, FPCALL_W32));
  rtl_sext(s, ddest, ddest, 4);
}

def_EHelper(fcvt_wu_s) {
  rtl_fpcall(s, ddest, dsrc1, rz, FPCALL_CMD(FPCALL_FToU32, FPCALL_W32));
  rtl_sext(s, ddest, ddest, 4);
}

def_EHelper(fcvt_l_s) {
  rtl_fpcall(s, ddest, dsrc1, rz, FPCALL_CMD(FPCALL_FToI64, FPCALL_W32));
}

def_EHelper(fcvt_lu_s) {
  rtl_fpcall(s, ddest, dsrc1, rz, FPCALL_CMD(FPCALL_FToU64, FPCALL_W32));
}
