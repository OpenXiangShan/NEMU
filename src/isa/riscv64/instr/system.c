...
__attribute__((cold))
int rtl_sys_slow_path(Decode *s, rtlreg_t *dest, const Operand *src1, uint32_t id, rtlreg_t *jpc) {
  ...
  if (funct3 == 0) {
    ...
    if (id == 0) { // ecall
      ...
      rtl_hostcall(s, HOSTCALL_PRIV, jpc, src1->preg, id);
    }
  ...
  rtl_hostcall(s, HOSTCALL_CSR, s0, NULL, id);
  int imm = funct3 & 0x4;
  int op  = funct3 & 0x3;
  if (imm) rtl_li(s, s1, src1->imm);
  else rtl_mv(s, s1, src1->preg);
  switch (op) {
    case 2: rtl_or(s, s1, s0, s1); break;
    case 3: rtl_not(s, s1, s1); rtl_and(s, s1, s0, s1); break;
  }
  rtl_hostcall(s, HOSTCALL_CSR, NULL, s1, id);
  rtl_mv(s, dest, s0);
  return 0;
}
