...
#define INSTR_TERNARY(f) \
  f(add) f(sub) f(slt) f(sltu) f(nemuand) f(nemuor) f(nemuxor) f(nor) f(sll) f(srl) f(sra) \
  f(addi) f(slti) f(sltui) f(nemuandi) f(nemuori) f(xori) f(slli) f(srli) f(srai) f(movz) f(movn) \
