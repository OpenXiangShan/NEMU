...
void init_audio() {
  ...
  add_pio_map("audio", CONFIG_AUDIO_CTL_PORT,
          (uint8_t *)audio_base, space_size, audio_io_handler);
  add_mmio_map("audio", CONFIG_AUDIO_CTL_MMIO,
          (uint8_t *)audio_base, space_size, audio_io_handler);
#ifndef __ICS_EXPORT
  ...
  add_mmio_map("audio-sbuf", CONFIG_SB_ADDR, (uint8_t *)sbuf, CONFIG_SB_SIZE, NULL);
}
