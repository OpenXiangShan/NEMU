...
/* device interface */
void add_pio_map(const char *name, ioaddr_t addr, void *space, uint32_t len, int skip_ref_flag, io_callback_t callback) {
...
uint32_t pio_read(ioaddr_t addr, int len) {
  ...
  int mapid = find_mapid_by_addr(maps, nr_map, addr, MMIO_READ);
#ifdef CONFIG_PA
  assert(mapid != -1);
#else
...
void pio_write(ioaddr_t addr, int len, uint32_t data) {
  assert(addr + len - 1 < PORT_IO_SPACE_MAX);
  int mapid = find_mapid_by_addr(maps, nr_map, addr, MMIO_WRITE);
#ifdef CONFIG_PA
  assert(mapid != -1);
#else
  if (mapid == -1) return;
